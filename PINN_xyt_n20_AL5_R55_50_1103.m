...
for i=1:length(coor_ind_ori)
    ...
    if(mod(i,ylen)==0 || i > ylen*(xlen-1) || mod(i-1,ylen+1)==0)
...
M1 = mod(Smpl-1,ylen)+1;%x
M2 = floor((Smpl-1)/ylen)+1;%y
M = [M1',M2'];
...
for epoch = 1:numEpochs
    ...
    v = randperm(tlen,numTimes); % Example vector v
    startIndices = (v - 1) * xlen*ylen + 1; % Starting indices for each submatrix
    ranges = startIndices.' + (0:xlen*ylen-1);
