function [loss,lossF,lossU,loss_s,loss_b,LAM1,LAM2,gradients, vU, PDEterms] = modelLoss_2D_atten_var_wuSVD_noLossb(parameters,X,Y, T,X0, Y0, T0,U0,lenT,C,alpha,wu,addon)
...
f = Utt +repmat(reshape(LAM1,1,900),1,(lenT+1)).*(Uxx+Uyy)+repmat(reshape(LAM2,1,900),1,(lenT+1)).*Ut; %repmat(parameters.lambda1,1,(140*4+1)).*Ut +
...
for i=1:size(LAM1,1)
    for j=1:size(LAM1,2)
        loss_s = loss_s + 1*relu(LAM1(i,j)) + 1*relu(-LAM2(i,j)) ;%+10*relu(LAM2(i,j));
    end
end
...
if(~isempty(addon))
    for i=1:size(addon,1)
        ...
        loss_b = loss_b+mse(LAM2(coor(1),coor(2)), alpha(coor(1),coor(2)),'DataFormat','CB');
    end
end
